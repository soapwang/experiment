...
def load_data_and_labels_movie():
    x_text=[]
    y = []
    folder_perfix = 'data/'
    x_pos = list(codecs.open(folder_perfix+"pos.txt","r",'utf-8').readlines())
    for i in x_pos:
        x_text.append(i)
        y.append("pos")
    x_neg = list(codecs.open(folder_perfix+"neg.txt","r","utf-8").readlines())
    for j in x_neg:
        x_text.append(j)
        y.append("neg")
    # x_text = x_pos+x_neg
...
def load_data():
    ...
    sentences, labels, test_size = load_data_and_labels_movie()
